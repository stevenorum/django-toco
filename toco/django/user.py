...
class User(Object):
    ...
    def active_session_tokens(self):
        ...
        return [SessionToken(id=i['id']) for i in response['Items'] if not i['expired']]
    ...
    def active_password_reset_requests(self):
        ...
        return [PasswordResetRequest(id=i['id']) for i in response['Items'] if not i['expired']]
...
class SessionToken(Object):
    ...
    def still_alive(self):
        return not self.expired and self.expiry > int(time.time())
    ...
    def keepalive_if_requested(self):
        ...
        """
        if self.auto_extend and not self.expired:
    ...
    def get_user_and_session(uuid):
        ...
        if token.expiry < time.time() or token.expired:
            return None, None
        else:
